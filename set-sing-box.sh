...
initall() {
    ...
    # 写入地点时区配置文件
    sudo cat <<20241204 | sudo tee /etc/timezone
Asia/Shanghai
20241204
    sudo cat <<20241204 | sudo tee /etc/cron.daily/ntpdate
...
20241204
    ...
    # 将简体中文字符集支持写入到环境变量
    cat <<20241204 | sudo tee /etc/default/locale
LANGUAGE=zh_CN.UTF-8
LC_ALL=zh_CN.UTF-8
LANG=zh_CN.UTF-8
LC_CTYPE=zh_CN.UTF-8
20241204
    cat <<20241204 | sudo tee -a /etc/environment
export LANGUAGE=zh_CN.UTF-8
export LC_ALL=zh_CN.UTF-8
export LANG=zh_CN.UTF-8
export LC_CTYPE=zh_CN.UTF-8
20241204
    cat <<20241204 | sudo tee -a $HOME/.bashrc
export LANGUAGE=zh_CN.UTF-8
export LC_ALL=zh_CN.UTF-8
export LANG=zh_CN.UTF-8
export LC_CTYPE=zh_CN.UTF-8
20241204
    cat <<20241204 >>$HOME/.profile
export LANGUAGE=zh_CN.UTF-8
export LC_ALL=zh_CN.UTF-8
export LANG=zh_CN.UTF-8
export LC_CTYPE=zh_CN.UTF-8
...
getAndStart() {
    ...
    *)
    ...
    # 创建目录 /opt/sing-box
    #sudo mkdir -pv /opt/${FILE_NAME%%_${VERSION#v}_$(uname -s)_$ARCH.deb}
    sudo mkdir -pv /opt/sing-box
    # 安装文件
    ...
    # 下载文件，可续传并打印进度
    wget --verbose --show-progress=on --progress=bar --hsts-file=/tmp/wget-hsts -c "$URI_DOWNLOAD" -O $FILE_NAME
    # 创建目录 /opt/cloudflared
    #sudo mkdir -pv /opt/${FILE_NAME%%-$(uname -s)-$ARCH.deb}
    sudo mkdir -pv /opt/cloudflared
    # 安装文件
    sudo dpkg -i $FILE_NAME
    # 删除文件
    rm -fv $FILE_NAME
    ...
    cat <<20241204 | sudo tee /etc/sing-box/config.json >/dev/null
{
  "log": {
    "disabled": false,
    "level": "info",
...
20241204
    ...
    sudo nohup sing-box -D /var/lib/sing-box -C /etc/sing-box run >/opt/sing-box/sing-box.log 2>&1 & disown
    sleep 1 ; cat /opt/sing-box/sing-box.log
    # 杀死 sing-box
    #sudo kill -9 $(sudo ps -ef | grep -v grep | grep sing-box | awk '{print $2}')
    sudo killall sing-box
    # 再次后台启用 sing-box 获得隧穿日志并脱离 shell 终端寿命，写入 tcp 协议 vmess 端口
    sudo nohup sing-box -D /var/lib/sing-box -C /etc/sing-box run >/opt/sing-box/sing-box.log 2>&1 & disown
    # 后台启用 cloudflared 获得隧穿日志并脱离 shell 终端寿命
    sudo nohup cloudflared tunnel --url http://localhost:$VM_PORT --no-autoupdate --edge-ip-version auto --protocol http2 >/opt/cloudflared/cloudflared.log 2>&1 & disown
    sleep 1 ; cat /opt/cloudflared/cloudflared.log
    # 杀死 cloudflared
    ...
    sudo nohup cloudflared tunnel --url http://0.0.0.0:$VM_PORT --no-autoupdate --edge-ip-version auto --protocol http2 >/opt/cloudflared/cloudflaredvm.log 2>&1 & disown
    
    # 睡 7 秒，让 cloudflared 充分运行
    sleep 7
    # sing-box 客户端配置所需变量
    # 出站代理名
    ...
    # vmess 域名
    CLOUDFLARED_DOMAIN_VM="$(cat /opt/cloudflared/cloudflaredvm.log | grep trycloudflare.com | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')"
    # vmess 端口
    ...
    # 下载文件，可续传并打印进度
    wget --verbose --show-progress=on --progress=bar --hsts-file=/tmp/wget-hsts -c "$URI_DOWNLOAD" -O $FILE_NAME
    # 创建目录 /opt/CloudflareST
    sudo mkdir -pv /opt/${FILE_NAME%%_$(uname -s)_$ARCH.tar.gz}
    # 解压项目到目录 /opt/CloudflareST
    sudo tar xzvf $FILE_NAME -C /opt/${FILE_NAME%%_$(uname -s)_$ARCH.tar.gz}
    # 执行测速命令，返回优选 ip
    cd /opt/${FILE_NAME%%_$(uname -s)_$ARCH.tar.gz}
    ...
    cd -
    # 删除文件
    sudo rm -rfv $FILE_NAME /opt/${FILE_NAME%%_$(uname -s)_$ARCH.tar.gz}
    # 浏览器
    BROWSER=chrome
    # VMESS 二维码生成扫描文件
    ...
    cat <<20241204 | sudo tee client-nekobox-config.yaml >/dev/null
port: 7891
socks-port: 7892
mixed-port: 7893
...
20241204
    ...
    cat <<20241204 | sudo tee client-sing-box-config.json >/dev/null
{
  "log": {
    "level": "debug",
    "timestamp": true
...
20241204
    ...
    # 写入 result.txt
    cat <<20241204 | sudo tee result.txt >/dev/null
